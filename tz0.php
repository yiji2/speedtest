...
function get_cpuinfo()
{
	...
	@preg_match_all("/CPU\s+implementer\s{0,}\:+\s{0,}(.+?)([\r\n]+)/s", $str, $cpu_implementer);
	@preg_match_all("/CPU\s+part\s{0,}\:+\s{0,}(.+?)([\r\n]+)/s", $str, $cpu_part);
	...
	if (is_array($model[1])) {
		...
		if (isset($cpu_implementer[1][0]) && $cpu_implementer[1][0] == '0x41') {
			$info['model'] = 'ARM ' . $arm_part[$cpu_part[1][0]];
		}
...
function get_boardinfo()
{
	...
	if ($disks = @scandir('/dev/disk/by-id')) {
		if ($names=array_filter($disks, function($k) { return $k[0] != '.' && strpos($k, 'DVD-ROM') === false; })) {
...
<table>
	...
	<td colspan="3">
		<?php if (isset($cpuinfo['model'])) : ?> <?php echo $cpuinfo['model']; ?><?php endif; ?>
		<?php if (isset($cpuinfo['frequency'])) : ?> | <?php __('Frequency'); ?>: <?php echo $cpuinfo['frequency']; ?> MHz<?php endif; ?>
		<?php if (isset($cpuinfo['l2cache'])) : ?> | <?php __('L2 Cache'); ?>：<?php echo $cpuinfo['l2cache']; ?><?php endif; ?>
		<?php if (isset($cpuinfo['bogomips'])) : ?> | Bogomips: <?php echo $cpuinfo['bogomips']; ?> ×<?php echo $cpuinfo['num'];?><?php endif; ?>
	</td>
	</tr>
	<tr>
...
<table class="table table-striped table-bordered table-hover table-condensed">
	<tr>
	<td><a href="mailto:chdoqsuca@mozmail.com?subject=Dedipath Prober Feedback">Feedback?</a></td>
